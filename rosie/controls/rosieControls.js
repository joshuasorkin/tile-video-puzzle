...
class PlayerController {
  ...
  update(deltaTime, cameraRotation) {
    ...
    if (this.keys['KeyW'] || this.keys['ArrowUp']) { // Forward
      moveX += forward.x;
      moveZ += forward.z;
    }
    if (this.keys['KeyS'] || this.keys['ArrowDown']) { // Backward
      moveX -= forward.x;
      moveZ -= forward.z;
    }
    // Strafe is disabled as per user request to use them for rotation
    /*
    if (this.keys['KeyA']) { // Left Strafe
      moveX -= right.x;
      moveZ -= right.z;
    }
    if (this.keys['KeyD']) { // Right Strafe
      moveX += right.x;
      moveZ += right.z;
    }
    */
...
class FirstPersonCameraController {
  ...
  setupMouseControls() {
    this.keys = {}; // Local key state for rotation
